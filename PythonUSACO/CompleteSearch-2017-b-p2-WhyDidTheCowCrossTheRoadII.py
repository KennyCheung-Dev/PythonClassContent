...
def UpdatePairs(letter):
    for i in range(currentCrossing.index(letter), len(currentCrossing)):
        for j in range(i + 1, len(currentCrossing)):
            if not (currentCrossing[i], currentCrossing[j]) in crossedPair and \
                    not (currentCrossing[j], currentCrossing[i]) in crossedPair:
                crossedPair.add((currentCrossing[i], currentCrossing[j]))

for i in cross:
    if i in currentCrossing:
        UpdatePairs(i)
        currentCrossing.remove(i)
    else:
        currentCrossing.append(i)

print(len(crossedPair))